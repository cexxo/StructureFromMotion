...
void FeatureMatcher::exhaustiveMatching()
{
  ...
  {
    cv::Mat img_i = readUndistortedImage(images_names_[i]);
    for( int j = i + 1; j < images_names_.size(); j++ )
    {
      std::cout<<"Matching image "<<i<<" with image "<<j<<std::endl;
      cv::Mat img_j = readUndistortedImage(images_names_[j]);
      ...
      /////////////////////////////////////////////////////////////////////////////////////////
        ...
        cv::Mat des1 , des2, E, H, mask_E, mask_H;
        
        std::vector<cv::DMatch> E_matches, H_matches;
        
        descriptors_[i].convertTo(des1,CV_32F);
        descriptors_[j].convertTo(des2,CV_32F);
        matcher.match(des1, des2, matches);
        std::vector<cv::Point2f> match_i, match_j;
        for(auto& match: matches){
            match_i.push_back(features_[i][match.queryIdx].pt);
            match_j.push_back(features_[j][match.trainIdx].pt);
        }
        E = cv::findEssentialMat(match_i,match_j,new_intrinsics_matrix_,cv::RANSAC,0.999,1.0,mask_E);
        H = cv::findHomography(match_i,match_j, cv::RANSAC, 1.0, mask_H);
        /*
        for(size_t k=0;k<matches.size();k++){
            if(mask_E.at<uchar>(k) || mask_H.at<uchar>(k)){
        ...
        for(size_t k=0; k<matches.size(); k++){
            if(mask_E.at<uchar>(k)){
                E_matches.push_back(matches[k]);
            }
            if(mask_H.at<uchar>(k)){
                H_matches.push_back(matches[k]);
            }
        }
        
        if(E_matches.size() >= H_matches.size()){
            inlier_matches=E_matches;
        } else{
            inlier_matches=H_matches;
        }
