#include "features_matcher_dnn.h"
...
FeatureMatcherDNN::FeatureMatcherDNN(cv::Mat intrinsics_matrix, cv::Mat dist_coeffs, double focal_scale)
{
...
cv::Mat FeatureMatcherDNN::readUndistortedImage(const std::string& filename )
{
...
void FeatureMatcherDNN::extractFeatures()
{
...
void FeatureMatcherDNN::exhaustiveMatching()
{
  std::vector<cv::DMatch> matches, inlier_matches;
  std::ofstream ofs;
  ofs.open("output_py.txt", std::ofstream::out | std::ofstream::trunc);
  ofs.close();
  ofs.open("color_py.txt", std::ofstream::out | std::ofstream::trunc);
  ofs.close();
  for( int i = 0; i < images_names_.size() - 1; i++ )
  {
    ...
    for( int j = i + 1; j < images_names_.size(); j++ )
    {
      ...
      /////////////////////////////////////////////////////////////////////////////////////////
      std::stringstream ss;
      ss << "python3 ../feature_ext.py " << images_names_[i] << " " << images_names_[i] << " " << i << " " << j;
...
void FeatureMatcherDNN::writeToFile ( const std::string& filename, bool normalize_points ) const
{

  //////////////////////////////////////////////////////////////////////////////////////////////////
  std::ifstream file;
  file.open("output_py.txt");
  std::string line;
  ...
  int total_number_of_observation = 0;
  if(file.is_open()){
      while(std::getline(file,line)){
        total_number_of_observation++;
      }
    fprintf(fptr, "%d %d %d\n", images_names_.size(), total_number_of_observation, total_number_of_observation);
    file.close();
    file.open("output_py.txt");
    while(std::getline(file,line)){
        fprintf(fptr, "%s\n",line.c_str());
    ...
    file.close();
    file.open("color_py.txt");
    while(std::getline(file,line)){
        fprintf(fptr, "%s\n",line.c_str());
      }
    fclose(fptr);
  }else{
    std::cout << "could not read python output file..." << std::endl;
  }
  //file.close();
  /////////////////////////////////////////////////////////////////////////////////////////////////
}

void FeatureMatcherDNN::testMatches( double scale )
{
...
void FeatureMatcherDNN::setMatches( int pos0_id, int pos1_id, const std::vector<cv::DMatch> &matches )
{
