...
bool BasicSfM::incrementalReconstruction(int seed_pair_idx0, int seed_pair_idx1) {
    ...
    if (cv::countNonZero(inlier_mask_E) > cv::countNonZero(inlier_mask_H)) {
        cv::recoverPose(E, points0, points1, intrinsics_matrix, init_r_mat, init_t_vec,
                        inlier_mask_E);
    ...
    for (int iter = 1; iter < num_cam_poses_ - 1; iter++) {
            ...
            if (cam_pose_optim_iter_[cam_idx] > 0) {
                for (auto const &co_iter: cam_observation_[cam_idx]) {
                    ...
                    if (pts_optim_iter_[pt_idx] == 0 &&
                        ...
                        proj_mat0=cv::Mat::zeros(3,4, CV_64F);
        ...
        //Method with bounding boxes
        cout << "Using method A" << endl;
        int X=50,Y=50, Z=50, K=scene_pts.size()/100;
        for (int k=0; k<scene_pts.size(); k++){
            int count = 0;
            for (int j=0; j<scene_pts.size(); j++) {
                if (abs(scene_pts[k].x - scene_pts[j].x) < X &&
                    abs(scene_pts[k].y - scene_pts[j].y) < Y &&
                    abs(scene_pts[k].z - scene_pts[j].z) < Z) {
                    count++;
                }
            }
            if (count < K) {
        ...
        /*cout << "Using method B" << endl;
        cv::Mat pointMat(3, scene_pts.size(), CV_64F, 0.0);
        cv::Mat covarMat, eigenVal, eigenVec, mean(3,1,CV_64F,0.0);
        for (int i=0; i < scene_pts.size(); i++) {
            pointMat.at<double>(0,i) = scene_pts[i].x;
            mean.at<double>(0,0) += scene_pts[i].x;
            pointMat.at<double>(1,0) = scene_pts[i].y;
            mean.at<double>(1,0) += scene_pts[i].y;
            pointMat.at<double>(2,i) = scene_pts[i].z;
            mean.at<double>(2,0) += scene_pts[i].z;
        }
        mean.at<double>(0,0) /= scene_pts.size();
        mean.at<double>(1,0) /= scene_pts.size();
        mean.at<double>(2,0) /= scene_pts.size();
    
        cv::calcCovarMatrix(pointMat,  covarMat, mean, cv::COVAR_COLS);
        cout << "covarMat" << endl;
        cv::eigenNonSymmetric(covarMat, eigenVal, eigenVec);

        cv::PCA(eigenVal, cv::noArray(), cv::PCA::DATA_AS_ROW);
        ...
        for (int i=0; i<2; i++) {
            cout << "eigen values:" << eigenVal.at<double>(i,0) << endl;
        }

        // NOTE: threshold in this line depends on the dataset
        if ((eigenVal.at<double>(0,0) / eigenVal.at<double>(1,0)) > 100 || (eigenVal.at<double>(0,0) / eigenVal.at<double>(1,0)) < 0.01) return false;*/
    ...
    while (keep_optimize) {
        ...
        for (int i_obs = 0; i_obs < num_observations_; i_obs++) {
            ...
            if (cam_pose_optim_iter_[cam_pose_index_[i_obs]] > 0 && pts_optim_iter_[point_index_[i_obs]] > 0) {
                ...
                double* observations=observations_.data()+(i_obs*2);
                // Initialization of cost function
                ceres::CostFunction *cost_function = ReprojectionError::Create(observations[0],observations[1]);

                // Adding residual block
                problem.AddResidualBlock(cost_function, new ceres::CauchyLoss(2 * max_reproj_err_), cameras, points ) ;
                /////////////////////////////////////////////////////////////////////////////////////////
        ...
        if (n_cheirality_violation > max_outliers_) {
            std::cout << "****************** OPTIM CHEIRALITY VIOLATION for " << n_cheirality_violation
                      << " points : redoing optim!!" << std::endl;
